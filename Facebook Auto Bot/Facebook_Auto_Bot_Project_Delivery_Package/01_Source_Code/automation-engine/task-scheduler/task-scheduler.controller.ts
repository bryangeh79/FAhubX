...
export class TaskSchedulerController {
  ...
  async createTask(@Body() createTaskDto: CreateTaskDto): Promise<TaskResponseDto> {
    const task = await this.taskSchedulerService.scheduleTask(createTaskDto as any);
  ...
  async getTasks(
    ...
    // 这里应该实现分页查询逻辑
    // 简化处理：返回所有任务
    const tasks = await this.taskSchedulerService.getPendingTasks();
    const runningTasks = await this.taskSchedulerService.getRunningTasks();
    
    const allTasks = [...tasks, ...runningTasks];
    ...
    if (accountId) {
      filteredTasks = filteredTasks.filter(task => task.accountId === accountId);
    }
