...
export class FacebookAccountsService {
  ...
  async create(
    ...
    const existingAccount = await this.facebookAccountsRepository.findOne({
      where: { facebookId: createFacebookAccountDto.facebookId },
      withDeleted: true,
    });

    if (existingAccount) {
      if (existingAccount.deletedAt) {
        throw new ConflictException('该Facebook账号已被删除，请联系管理员恢复');
      }
    ...
    try {
      // 加密访问令牌
      const encryptedAccessToken = this.encryptData(createFacebookAccountDto.accessToken);
      const encryptedRefreshToken = createFacebookAccountDto.refreshToken
        ? this.encryptData(createFacebookAccountDto.refreshToken)
        : null;

      // 创建账号实体
      const account = this.facebookAccountsRepository.create({
        userId,
        facebookId: createFacebookAccountDto.facebookId,
        name: createFacebookAccountDto.name,
        email: createFacebookAccountDto.email,
        accessToken: encryptedAccessToken,
        accessTokenExpiresAt: new Date(createFacebookAccountDto.accessTokenExpiresAt),
        refreshToken: encryptedRefreshToken,
        accountType: createFacebookAccountDto.accountType,
        profilePicture: createFacebookAccountDto.profilePicture,
        coverPhoto: createFacebookAccountDto.coverPhoto,
        followersCount: createFacebookAccountDto.followersCount,
        followingCount: createFacebookAccountDto.followingCount,
  ...
  async getStats(userId: string): Promise<{
    ...
    return {
      ...
      expiredAccounts: allAccounts.filter(a => a.status === 'expired').length,
